...
unsigned int  CompletionThread(LPVOID pComPort);
...
//queue<IO_DATA*> g_Queue_recv_buf;
...
*/
		...
		//아니면 함수 자체를 unsigned int __stdcall로 바꿔도 됨  printf("CPU개수 : %d입니다, Thread개수: %d\n ",SystemInfo.dwNumberOfProcessors,g_Num_Of_Thread);
	 /*
	...
	bool b_Retval =FALSE;
	 ...
	 {	
		...
		retval = WSARecv(PerHandleSockData->hClientSock, //연결 소켓을 가리키는 소켓 지정 번호 clientSock
			...
			memcpy(realData.buffer ,PerIoData->buffer,MEG_LENGHT);
			realData.sock = PerHandleSockData->hClientSock;
			
			g_Queue_recv_buf.push(realData);
			//10 21//10 21//10 21//10 21//10 21//10 21//10 21//10 21
